...
# For TAPEX model
from transformers import BartConfig, BartForConditionalGeneration, TapexTokenizer
...
from models.rag.itr_rag import ITRRagAdditionRowWiseModel, ITRRagModel
from models.rag.itr_rag_reduce import (
    ITRRagReduceMixModel,
    ITRRagReduceModel,
    ITRRagReduceRowWiseModel,
)
...
class ITRRAGExecutor(BaseExecutor):
    ...
    def _generative_step(self, sample_batched, batch_idx):
        ...
        for index, i in enumerate(labels):
            ...
            original_table_overflow = generator_input_sub_tables[index][0][
            ...
            for doc, doc_prediction in zip(
                ...
                table_entry += [
                    "[" + sample_batched["tables"][index]["id"] + "] " + col_row_info,
                    doc_prediction,
                ]
            ...
            if num_retrieved <= max_K:
                for doc, doc_score in zip(retrieved_docs[index], doc_scores[index]):
                    present_str = (
                        str(doc["header"])
                        if doc.get("sub_type", "column_wise") == "column_wise"
                        else str(doc["sub_row_indice"])
                    )
                    table_entry += [f'[{doc["id"]}] {present_str}', doc_score]
                for i in range(
                    max(self.config.model_config.Ks) - len(retrieved_docs[index])
                ):
                    table_entry += ["", 0]
            else:
    ...
    def evaluate_outputs(self, step_outputs, mode="test"):
        ...
        for step_output in step_outputs:
            ...
            batch_generator_input_sub_tables += step_output[
                "generator_input_sub_tables"
            ]
