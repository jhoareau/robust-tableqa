...
from transformers.models.rag.retrieval_rag import (
    Index,  # CustomHFIndex, CanonicalHFIndex
)
...
class RagModel(pl.LightningModule):
    ...
    def __init__(self, config: EasyDict, data_loader) -> None:
        ...
        self.use_colbert = (
            True
            if self.config.model_config.QueryEncoderModelClass == "ColBERT"
            else False
        )
    ...
    def init_retrieval(self):
        ...
        if self.use_colbert:
            ...
            self.table_dict = (
                self.table_dataset.add_column(
                    "table_index", list(range(len(self.table_dataset)))
                )
                .to_pandas()
                .set_index("table_id", drop=False)
                .to_dict(orient="index")
            )

            self.table_dict_by_index = (
                self.table_dataset.add_column(
                    "table_index", list(range(len(self.table_dataset)))
                )
                .to_pandas()
        ...
        if self.config.model_config.index_files.index_passages_path == "":
            ...
            self.index = CanonicalHFIndex(
                ...
                if self.config.model_config.index_files.index_dummy
                else False,
            )
    ...
    def main_retrieve(
        ...
        **kwargs,
        ...
        dont_add_reference_table = (
            "dont_add_reference_table" in self.config.model_config.modules
        )
            ...
            if pos_item_ids is not None and not dont_add_reference_table:
                ...
                if not has_reference_table:
                    ...
                    if self.use_colbert:
                        ...
                        pos_item_embedding_mask = torch.Tensor(emb_tuple[1]).unsqueeze(
                            0
                        )
                    ...
                    if self.use_colbert:
                        ...
                        score = self.question_encoder.score(
                            question_hidden_states[[b]],
                            pos_item_embeddings,
                            pos_item_embedding_mask,
                        )
                        # print('score', score)
                        ...
                        modified_doc_scores[b, 0] = torch.matmul(
                            question_hidden_states[b],
                            torch.Tensor(pos_item["embeddings"]).to(self.device),
                        )
    ...
    def prepare_inputs_for_generator(
        ...
        encoding = self.generator_tokenizer(
            query=[
                text_table_tuple[0]
                for text_table_tuple in extended_input_text_sequences
            ],
            table=[
                pd.DataFrame.from_records(
                    text_table_tuple[1]["rows"], columns=text_table_tuple[1]["header"]
                )
                for text_table_tuple in extended_input_text_sequences
            ],
            padding="longest",
    ...
    def forward(
        ...
        **kwargs,
        ...
        if "add_binary_labels_as_prompt" in self.config.model_config.modules:
            ...
            labels = [
                [
                    (label if label != self.generator_tokenizer.pad_token_id else -100)
        ...
        **kwargs,
        ...
        if "rag_decoding" in self.config.model_config.modules:
            ...
            for index in range(batch_size):
                ...
                individual_input_ids = generator_inputs.generator_input_ids[
                    index * n_docs : (index + 1) * n_docs
                ].repeat(
                    num_candidates, 1
                )  # (num_candidates*n_docs, max_len)

                individual_attention_mask = generator_inputs.generator_attention_mask[
                    index * n_docs : (index + 1) * n_docs
                ]
                ...
                loss_target = torch.cat(
                    [
                        loss_target[:, 1:],
                        loss_target.new(loss_target.shape[0], 1).fill_(
                            self.generator.config.pad_token_id
                        ),
                    ],
                    1,
                ...
                for doc_index in range(n_docs):
                    ...
                    loss_of_beams = -rag_loss[
                        doc_index * n_beams : (doc_index + 1) * n_beams
                    ]
                    ...
                    doc_loss.append(loss_of_beams[top_index])
                doc_loss = torch.FloatTensor(doc_loss)
                # print('doc_loss', doc_loss)

                loss_with_doc_scores.append(doc_loss)
            ...
            if "add_binary_labels_as_prompt" in self.config.model_config.modules:
                for b in range(batch_size):
                    loss_mask = [
                        "no. " in generated_str
                        for generated_str in generation_outputs_decoded[
                            b * n_docs : (b + 1) * n_docs
                        ]
                    ]
                    for i in range(n_docs):
                        if loss_mask[i]:
                            loss_with_doc_scores[b, i] = (
                                loss_with_doc_scores[b, i] - 100.0
                            )
    ...
    def get_loss(
        ...
        if "mask_non_reference_tables" in self.config.model_config.modules:
            smoothed_nll_loss = smoothed_nll_loss * retrieval_labels.unsqueeze(-1).to(
                self.device
            )
        ...
        if self.config.model_config.loss_ratio.additional_loss != 0:
            if retrieval_labels is not None:
                ...
                first_token_target = target.reshape(batch_size, n_docs, -1)[
                    :, :, target_index
                ]
    ...
    def get_rag_nll(
        ...
        if ignore_index_pad_mask.any():
            # fill -100 to be 0, avoid indexing error using gather
            target.masked_fill_(
                ignore_index_pad_mask, self.generator.config.pad_token_id
            )

        n_docs = (
            n_docs
            if n_docs is not None
            else self.config.data_loader.additional.num_knowledge_passages
        )
